...
echo "创建项目目录结构..."
mkdir -p $PROJECT_DIR/{app/{core,models,routers,static,templates},scripts/templates}
cd $PROJECT_DIR
...
# 模板定义
TEMPLATES = {
    "model.py.j2": """from sqlalchemy import Column, {field_types}
...
""",
    ...
    "router.py.j2": """from fastapi import APIRouter
...
<html lang="ja">
    ...
    <body class="gradient-bg">
        ...
        <div class="absolute inset-0 opacity-10">
            <div class="absolute inset-0" style="background-image: radial-gradient(circle at 25% 25%, rgba(255,255,255,0.2) 0%, transparent 50%), radial-gradient(circle at 75% 75%, rgba(255,255,255,0.2) 0%, transparent 50%);"></div>
        </div>
        ...
        <div class="relative z-10 min-h-screen">
            ...
            <!-- Mobile Header -->
            <div class="lg:hidden bg-white/90 backdrop-blur-md border-b border-white/20 sticky top-0 z-50">
                <div class="flex items-center justify-between p-4">
                    <h1 class="text-lg font-bold text-gray-800">{{{{ model_name }}}}</h1>
                    <button id="mobile-menu-btn" class="p-2 rounded-lg bg-purple-100 text-purple-600">
            ...
            <!-- Mobile Menu Overlay -->
            <div id="mobile-menu-overlay" class="lg:hidden fixed inset-0 bg-black/50 z-40 opacity-0 invisible transition-all duration-300"></div>
            
            <!-- Mobile Menu -->
            <div id="mobile-menu" class="lg:hidden fixed right-0 top-0 h-full w-80 bg-white shadow-xl z-50 mobile-menu">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-xl font-bold text-gray-800">メニュー</h2>
                        <button id="close-menu" class="p-2 rounded-lg text-gray-500">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <div class="space-y-4">
                        <a href="/{{{{ table_name }}}}/new" class="block w-full bg-gradient-to-r from-green-500 to-blue-600 text-white py-3 px-4 rounded-xl font-semibold text-center">
                            <i class="fas fa-plus mr-2"></i>新規作成
                        </a>
                        <button onclick="location.reload()" class="block w-full bg-gray-500 text-white py-3 px-4 rounded-xl font-semibold">
...
<html lang="ja">
    ...
    <body class="min-h-screen gradient-bg p-4 sm:p-6 lg:p-8">
...
<body class="bg-gradient-to-br from-purple-500 to-blue-600 min-h-screen flex items-center justify-center p-4">
    <div class="bg-white p-8 rounded-2xl shadow-lg max-w-md w-full space-y-6">
        <h1 class="text-2xl font-bold text-center text-gray-800">
            <i class="fas fa-lock mr-2 text-purple-600"></i>ログイン
        </h1>
        <form method="post" action="/login" class="space-y-4">
            <div>
                <label class="block text-gray-700 font-medium mb-1">ユーザーID</label>
...
<body class="bg-red-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 rounded shadow max-w-md text-center">
        <h1 class="text-2xl font-bold text-red-600 mb-4">アクセス拒否</h1>
        <p class="mb-6 text-gray-700">このページを表示する権限がありません。</p>
        <a href="/login" class="bg-red-500 text-white px-4 py-2 rounded">ログインページへ戻る</a>
    </div>
</body>
</html>
EOL
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-6 rounded shadow text-center">
        <h1 class="text-xl font-bold mb-2">ログアウトしました</h1>
        <a href="/login" class="bg-blue-500 text-white px-4 py-2 rounded">再ログイン</a>
    </div>
</body>
</html>
EOL
...
<body class="gradient-bg">
    ...
    <div class="relative z-10 min-h-screen">
        ...
        <div class="container mx-auto px-4 py-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <a href="/m_users" class="nav-card bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-6 text-white">
                    <div class="flex items-center">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const workClassChart = new Chart(workClassCtx, {
                type: 'pie',
                data: {
                    labels: [],
                    datasets: [{
            ...
            const monthlyWorkChart = new Chart(monthlyWorkCtx, {
                type: 'bar',
                data: {
                    labels: [],
                    datasets: [{
            ...
            htmx.on('htmx:afterSwap', function(evt) {
                if (evt.detail.target.id === 'chart-data-container') {
                    const data = JSON.parse(evt.detail.target.innerHTML);
                    
                    // 更新饼图
                    workClassChart.data.labels = data.workClass.labels;
                    workClassChart.data.datasets[0].data = data.workClass.data;
                    workClassChart.update();
                    
                    // 更新柱状图
                    monthlyWorkChart.data.labels = data.monthlyWork.labels;
                    monthlyWorkChart.data.datasets[0].data = data.monthlyWork.data;
                    monthlyWorkChart.update();
                }
            });
        });
    </script>
