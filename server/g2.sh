...
echo "创建 项目目录结构..."
mkdir -p $PROJECT_DIR/{app/{core,config,config/models,models,routers,static,static/locales,static/js,templates,templates/partials,templates/customer_view,utils},scripts/templates}
cd $PROJECT_DIR
...
TABLES = {
    ...
    "customer_view": {
        "base_model": "MCustomers",
        "abstract": True,
        "fields": []  # 可空，表示不生成字段，只继承
    },
    "work_view": {
        "base_model": "TWork",
        "abstract": True,
...
def generate():
    for table_name, config in TABLES.items():
        ...
        if "base_model" in config:
            # 视图模型：继承已有模型
            content = f"""from app.models.{config['base_model'].lower()} import {config['base_model']}

class {model_name}({config['base_model']}):
    __tablename__ = "{table_name}"
    {"__abstract__ = True" if config.get("abstract") else ""}
"""
            output_path = f"app/models/{table_name}.py"
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            with open(output_path, "w", encoding='utf-8') as f:
                f.write(content)

            print(f"[view model] Generated: {output_path}")
        else:
            # 普通表模型
            columns = []
            for field in config["fields"]:
            ...
            print(f"[model] Generated: {table_name}")

        # ------ 视图模板生成 ------
        if table_name.endswith("_view"):
            template_dir = f"app/templates/{table_name}"
            os.makedirs(template_dir, exist_ok=True)

            with open(f"{template_dir}/list.html", "w", encoding="utf-8") as f:
                f.write(f"""{{% extends "base_list.html" %}}
{{% set new_url = '#' %}}
{{% block header_title %}}
{model_name} 一覧
{{% endblock %}}""")

            with open(f"{template_dir}/works.html", "w", encoding="utf-8") as f:
                f.write(f"""{{% extends "base_list.html" %}}
{{% block header_title %}}
{{{{ customer.name }}}} の作業一覧
{{% endblock %}}
{{% set new_url = '/t_work/new?customer_id=' ~ customer.id %}}""")

            print(f"[view templates] Generated: {template_dir}/list.html, works.html")

    # ------ __init__.py 注册（不包括 abstract） ------
    with open("app/models/__init__.py", "a", encoding='utf-8') as f:
        f.write("\n# Import all models\n")
        for table_name, config in TABLES.items():
...
echo " customer_view 文件..."
cat > app/routers/customer_view.py << 'EOL'
from fastapi import APIRouter, Request, Depends, HTTPException
from fastapi.templating import Jinja2Templates
from sqlalchemy.orm import Session
from app.models import get_db
from app.models.customer_view import CustomerView
from app.models.work_view import WorkView
from app.core.auth import get_current_user

router = APIRouter(prefix="/customer_view", tags=["CustomerView"])
templates = Jinja2Templates(directory="app/templates")

@router.get("/", response_class=HTMLResponse)
async def list_customers(request: Request, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    items = db.query(CustomerView).all()
    fields = [col for col in CustomerView.__table__.columns if col.name != 'id']
    return templates.TemplateResponse("customer_view/list.html", {
        "request": request,
        "items": items,
        "fields": fields,
...
@router.get("/{customer_id}/works", response_class=HTMLResponse)
async def customer_works(customer_id: int, request: Request, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    customer = db.query(CustomerView).filter(CustomerView.id == customer_id).first()
    if not customer:
        raise HTTPException(status_code=404, detail="Customer not found")

    works = db.query(WorkView).filter(WorkView.customer_id == customer_id).all()
    fields = [col for col in WorkView.__table__.columns if col.name != 'id']

    return templates.TemplateResponse("customer_view/works.html", {
        "request": request,
        "customer": customer,
        "items": works,
        "fields": fields,
...
echo " app/models/customer_view.py..."
cat > app/models/customer_view.py << 'EOL'
from app.models.m_customers import MCustomers

class CustomerView(MCustomers):
    __tablename__ = "customer_view"
    __abstract__ = True

EOL

echo " app/models/work_view.py..."
cat > app/models/work_view.py << 'EOL'
from app.models.t_work import TWork

class WorkView(TWork):
    __tablename__ = "work_view"
    __abstract__ = True

EOL

echo " app/templates/customer_view/list.html..."
cat > app/templates/customer_view/list.html << 'EOL'
{% extends "base_list.html" %}
{% set new_url = '#' %}
{% block header_title %}
顧客ビュー一覧
...
# 路由注册 customer_view
from app.routers.customer_view import router as customer_view_router
app.include_router(customer_view_router)
